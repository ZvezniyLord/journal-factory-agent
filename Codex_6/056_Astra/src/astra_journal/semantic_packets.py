...
def article_semantic_instruction(packet: dict[str, Any]) -> str:
    return (
        ...
        "Return JSON only. Use EXACTLY one top-level envelope with keys: "
        "task,status,confidence,result,evidence,warnings. "
        "task must equal 'article_semantic_audit'; status must be one of "
        "ok/review/fail/unknown; confidence must be a number 0..1; "
        "evidence and warnings must be arrays. "
